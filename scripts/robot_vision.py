...
# 始能相机服务
def camera_service():
    rospy.wait_for_service("/ur5e/camera/enable")
    try:
        camera_client = rospy.ServiceProxy("/ur5e/camera/enable", set_int)
...
def camera_Recognition_service():
    rospy.wait_for_service("/ur5e/camera/recognition_enable")
    try:
        camera_Recognition_client = rospy.ServiceProxy("/ur5e/camera/recognition_enable",set_int)
...
if __name__ == "__main__":
    ...
    rospy.Subscriber("/ur5e/camera/recognition_objects", RecognitionObject, recognition_objects_callback)
